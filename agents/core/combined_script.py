...
from config import OPEN_API_KEY, MODEL_NAME
...
def process_yaml_files_and_call_llm(yaml_folder, question):
    ...
    print(f"Вызов агента для выбора файла с вопросом: {question}")
    ...
    print(f"Файл выбран: {selected_file}")
...
def process_yaml_and_answer(yaml_folder, file_name, original_question, memory, user_id):
    ...
    print(f"Агент Pathfinder получил запрос: {original_question}")
    logger.info("Агент Pathfinder начал обработку запроса")
    print("Агент Pathfinder начал обработку запроса")
    ...
    if "конкретика" in route_decision.lower():
        ...
        print("Агент Pathfinder решил, что запрос должен быть передан агенту Конкретика")
        ...
        print("Агент Pathfinder решил, что запрос должен быть передан агенту Продажник")
